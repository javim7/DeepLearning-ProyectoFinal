...
def obtener_h2h(dataModel, homeTeam, awayTeam):
    ...
    # Si no hay partidos anteriores, devolver 0 para ambos
    if partidos.empty:
        return 0, 0
...
def preparar_datos(jornada, partido, tabla, dataModel):
    ...
    HHWH, HHWA = obtener_h2h(dataModel, homeTeamMapping, awayTeamMapping)
    print(f"'{homeTeam}', '{awayTeam}', {PointsHome}, {PointsAway}, {GoalDiffHome}, {GoalDiffAway}, {HHWH}, {HHWA}")

    return homeTeamMapping, awayTeamMapping, PointsHome, PointsAway, GoalDiffHome, GoalDiffAway, HHWH, HHWA
...
if __name__ == "__main__":
    ...
    dataModel = pd.read_csv('modelos/data1.csv')
    ...
    # for jornada in range (1,2):
        ...
        for i, partido in partidos_jornada.iterrows():
            datos_partido = preparar_datos(jornada, partido, tabla, dataModel)
            # print(datos_partido)
            ...
            tabla_predicciones.to_csv(f'predicciones/jornadasPred1/{jornada}.csv', index=False)
